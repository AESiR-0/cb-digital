  ...
  return (
    <div className="w-full fixed top-0 z-50 bg-primary">
      <div className="flex items-center justify-between px-5 md:px-20 xl:px-40 2xl:px-60 h-20 text-white max-w-screen-2xl mx-auto">
        ...
        {/* Mobile Menu */}
        <div
          className={`fixed inset-0 bg-secondary flex flex-col items-center justify-center space-y-6 text-lg transition-transform ${isOpen ? "translate-x-0" : "-translate-x-full"
            } md:hidden`}
        >
          {navItems.map((item: NavItem, index) => (
            <Link
              ...
              className={`font-clash font-black ${item.link === pathName ? "text-accent" : "text-secondary-4"
